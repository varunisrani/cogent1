    ...
    try:
        ...
        if agent_mode == "mcp":
            try:
                ...
                # Signal end of text streaming and start of code display
                yield "___CODE_SECTIONS_FOLLOW___"
                
                # Send file contents for display
                if hasattr(st.session_state, 'mcp_file_contents') and st.session_state.mcp_file_contents:
                    for filename, content in st.session_state.mcp_file_contents.items():
                        # Only yield key files that users care most about
                        if filename in ['agents.py', 'tools.py', 'tasks.py', 'crew.py', 'main.py', 'run_agent.py']:
                            yield f"__FILE__{filename}__FILE__{content}"
    ...
    try:
        ...
        if result.get("success", False):
            ...
            if tool_mode == "single":
                ...
                response = f"""✅ **Successfully integrated MCP tool**
...
"""
            ...
            # Store file contents in session state for display
            if "file_contents" in result:
                st.session_state.mcp_file_contents = result.get("file_contents", {})
            
            # List files in the response
...
{result.get('message', result.get('error', 'An unknown error occurred.'))}
"""
        ...
        for message in st.session_state.messages:
            ...
            if message_type == "system":
                ...
                with st.chat_message("user" if message_type == "human" else "assistant", 
                    ...
                    if message_type == "ai" and "___CODE_SECTIONS_FOLLOW___" in message["content"]:
                        # Split content into message and code sections
                        parts = message["content"].split("___CODE_SECTIONS_FOLLOW___")
                        st.markdown(parts[0])  # Display the message part
                        
                        # Display code sections in expandable containers
                        if "code_sections" in message:
                            for filename, code in message["code_sections"].items():
            ...
            try:
                ...
                async for chunk in run_agent_with_streaming(user_input):
                    if chunk.startswith("__FILE__"):
                        # Parse file content
                        _, filename, content = chunk.split("__FILE__", 2)
                        code_sections[filename] = content
                        continue
                    
                    if chunk == "___CODE_SECTIONS_FOLLOW___":
                        # Now we'll show code sections
                        continue
                ...
                # After streaming is complete, show code sections in expandable containers
                if code_sections:
                    for filename, code in code_sections.items():
                        with st.expander(f"📄 {filename}"):
                            st.code(code, language="python")
                
                # Store both the response and code sections
                message_obj = {"type": "ai", "content": response_content}
                if code_sections:
                    message_obj["content"] += "___CODE_SECTIONS_FOLLOW___"
                    message_obj["code_sections"] = code_sections
