...
async def combined_adaptive_flow(
    ...
    try:
        ...
        if not tools_result.get("tools", []):
            ...
            return {"error": "No suitable MCP tools found for your request", "success": False}
        ...
        # Keep track of all files written
        files_written = [error_list_path, os.path.join(project_dir, "tools.py"), os.path.join(project_dir, "requirements.txt")]
        ...
        if template_results and len(template_results) > 0:
            ...
            # Track all template-generated files
            for filename, content in template_results.items():
            ...
            logger.info(
                f"Generated README.md at {readme_path}")

            # After files are generated, read their contents
            file_contents = {}
            ...
            return {
                ...
                "template_used": True,
                "files": files_written,
                "file_contents": file_contents,  # Add file contents to the result
                "tool_info": {
                    "tool_count": len(tool_class_names),
                    "tool_names": tool_class_names,
