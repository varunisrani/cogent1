...
# Configure enhanced logging for YouTube transcription operations
def setup_logging(service_name="youtube_transcription_service"):
    """Set up detailed logging for YouTube transcription operations"""
...
class YouTubeTranscriptionParams(BaseModel):
    """Parameters for YouTube transcription tool operations"""
    operation: str = Field(
        description="The transcription operation to perform via the YouTube API"
    )
    parameters: Dict[str, Any] = Field(
        default={},
        description="Parameters for the YouTube transcription operation"
    )

class YouTubeTranscriptMCPMCPTool(BaseTool):
    """Tool for interacting with YouTube transcription API using MCP"""
    name: str = "youtube_transcription_tool"
    description: str = "Extract and format transcripts from YouTube videos, supporting multiple languages and timestamping."
    args_schema: type[BaseModel] = YouTubeTranscriptionParams
    
    model_config = {"arbitrary_types_allowed": True}
    
    service_name: str = "youtube_transcription"
    ...
    def __init__(
        ...
        """Initialize the YouTube transcription MCP tool"""
        super().__init__()
        
        load_dotenv()
        
        self.service_name = "youtube_transcription"
    ...
    def _run(self, operation: str, parameters: Dict[str, Any] = None) -> str:
        """Run the MCP operation for YouTube transcription"""
...
"""
    ...
    async def _run_async(self, operation: str, parameters: Dict[str, Any]) -> str:
        """Run a YouTube transcription operation asynchronously"""
        start_time = time.time()
        self.logger.info("=" * 80)
        self.logger.info(f"OPERATION CALLED: {operation}")
        self.logger.info("-" * 80)
        ...
        while retry_count <= max_retries:
            try:
                self.logger.info("Connecting to YouTube transcription API...")
...
{formatted_transcript}"""
                ...
                result = f"Error executing YouTube transcription operation: {error_str}"
                break
    ...
    async def _validate_and_normalize_parameters(self, operation: str, parameters: Dict[str, Any], tool_details: Dict) -> Dict[str, Any]:
        """Validate and normalize parameters for YouTube transcription operations"""
            ...
            if "missing" in error_lower or "required" in error_lower:
                ...
                if problem_param in corrected_params:
                    ...
                    if problem_param == "url":
                        ...
                        if video_id:
                            corrected_params[problem_param] = f"https://www